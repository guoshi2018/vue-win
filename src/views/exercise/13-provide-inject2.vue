    ...
    <accordion-item caption="在 setup() 中使用 provide方法" show>
      <ul>
        <li>
          ...
          <span>geolocation跟踪:{{geolocation.longitude}},{{geolocation.latitude}}</span>
        </li>
        <li>
          <span>消费端修改:</span>
          location:<input type="text" v-model="location" /><br />
          longitude:<input type="number" v-model="geolocation.longitude" /><br />
        ...
        <li>
          <span>尽可能将对响应式 property 的所有修改限制在定义 provide 的组件内部</span>
...
<script lang="ts">
  ...
  export default {
    ...
    setup() {
      ...
      return {
        location, geolocation, updateLocation
      }
    },
