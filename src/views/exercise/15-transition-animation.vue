<template>
  <accordion-hull header="过渡&动画概述">
    <accordion-item caption="基于 class 的动画和过渡">
      <ol>
        <li>
          <span> Push this button to do something you shouldn't be doing: </span>
...
button:focus {
  outline: 1px dashed rgb(78, 19, 146);
  outline-offset: -5px;
}

.shake {
  animation: shake 0.82s cubic-bezier(0.36, 0.07, 0.19, 0.97) both;
  /*以下任意一个，实现对元素进行硬件加速, 所以可选 */
...
.unit {
  ...
  @include background-image(
    radial-gradient(center, ellipse cover, #222426 0%, #111111 100%)
  );
...
.ball {
  ...
  @include background-image(
    radial-gradient(center, ellipse cover, #959595 0%, #4e4e4e 79%, #494949 100%)
  );
  transform: translateZ(0);
  margin: 30px auto;
  position: relative;
