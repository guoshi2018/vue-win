<template>
  <accordion-hull
    header="列表渲染:v-for='item in items'和 v-for='item of items'"
  >
    <accordion-item
      caption="数组和内部的对象元素均使用v-for='item in items'"
      show
    >
      <ul>
    ...
    <accordion-item
      caption="数组更新检测。由于vue将一些数组方法进行了包裹，
                   使得它们也会触发视图更新，这些方法有：
      ...
      <ul>
        <li>
          ...
          <input
            type="button"
            value="unshift 2"
            @click="updateArray('unshift')"
          />
          <input
            type="button"
            value="splice 2"
            @click="updateArray('splice')"
          />
          <input type="button" value="sort" @click="updateArray('sort')" />
          <input
            type="button"
            value="reverse"
            @click="updateArray('reverse')"
          />
        </li>
      </ul>
    </accordion-item>
    <accordion-item
      caption="使用计算属性显示过滤的结果：筛选数学和语文分数均为奇数的人员记录"
    >
      <ul>
        <li v-for="(mark, idx) of oddChMathMarks" :key="idx">
          {{ idx + 1 }}:{{ mark.name }}
          <ul>
            <li v-for="(propValue, propName, j) of mark" :key="j">
              {{ j + 1 }}---{{ propName }}:{{ propValue }}
            </li>
          </ul>
        </li>
      </ul>
    ...
    <accordion-item
      caption="使用嵌套v-for循环，结合方法，展示嵌套数组的偶数元素。
    ...
    <accordion-item
      caption="类似于 v-if，你也可以利用带有 v-for 的 &lt;template&gt; 来循环渲染一段包含多个元素的内容。"
    >
      <ul>
    ...
    <accordion-item
      caption="v-for与v-if一同使用，可以把v-for移动到&lt;template&gt;中来,以轮显语文90分以上的人员"
    >
      <ul>
    ...
    <accordion-item
      caption="组件上使用v-for：官方文档用例，其中引用todo-item组件（有一点点改动）"
    >
      <ul>
        <li>
          <form v-on:submit.prevent="addNewTodo">
            ...
            <input
              type="submit"
              value="add"
              :disabled="plan.newTodoText.length == 0"
            />
            <ul>
              <TodoItem
                v-for="(todo, index) in plan.currentTodos"
                :key="todo.id"
                :title="todo.title"
                @remove="plan.currentTodos.splice(index, 1)"
              ></TodoItem>
            </ul>
          </form>
        </li>
    ...
    updateArray(flag: string): void {
      switch (flag) {
        ...
        case "splice":
          ...
          break;
        case "sort":
          this.marks.sort((m1: Mark, m2: Mark): number => m1.math - m2.math);
          break;
        case "reverse":
          this.marks.reverse();
          break;
