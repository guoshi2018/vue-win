...
<template>
  <accordion-hull header="Teleport,允许我们控制在 DOM 中哪个父节点下渲染了 HTML">
    <accordion-item caption="teleport技术">
      <dl>
        <dt>
          Vue 鼓励我们通过将 UI 和相关行为封装到组件中来构建 UI。我们可以将它们嵌套在另一个内部，
          以构建一个组成应用程序 UI 的树。
        ...
        <dd>
          <span>不采用teleport技术的first-modal-dialog,模态框只能以最近的relative祖先定位</span>
          <div style="position:relative;">
            <h2>
              Tooltips without Vue3 teleport technoligy!
            </h2>
            <div>
              <first-modal-dialog />
            </div>
        ...
        <dd>
          <span>
            给first-modal-dialog的模态部分div加一层 &lt;teleport to='body'&gt;外套,即可实现全屏定位,
