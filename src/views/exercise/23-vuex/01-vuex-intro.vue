...
<template>
  <accordion-hull header="Vuex介绍">
    ...
    <accordion-item caption="使用store提交mutation事件">
      <ul>
        <li>
          <input
            type="button"
            value="this.$store.commit(INCREMENT)"
            @click="demo1"
          />
        </li>
        <li>
        ...
        <li>
          <input
            type="button"
            value="this.$store.commit(ADD_COUNT, 15)载荷风格,载荷为简单number类型"
            @click="demo3"
          />
        </li>
        <li>
          <input
            type="button"
            value="this.$store.commit(ADD_TODO,{...})载荷风格,载荷为Todo对象类型"
            @click="demo4"
          />
        </li>
        <li>
    ...
    >
      <ul>
        <li>
          <input type="button" value="使用increment" @click="increment" />
          <input
            type="button"
            value="使用[INCREMENT]无效"
            @click="[INCREMENT]"
          />
        </li>
        <li>
        ...
        <li>
          <input type="button" value="使用add_count" @click="add_count(2819)" />
        </li>
        <li>
          <input
            type="button"
            value="使用add_using_object_style"
            @click="
              add_using_object_style({
                type: ADD_USING_OBJECT_STYLE,
                amount: 38,
                todo: {
                  id: 199,
    ...
    <accordion-item caption="action的映射用法,映射成组件methods之一">
      <ul>
        ...
        <li>
          ...
          >
          <input type="button" value="递增19" @click="act_add_count(19)" />
        </li>
      </ul>
    </accordion-item>
...
export default defineComponent({
  ...
  methods: {
    demo1() {
      this.$store.commit("increment"); //ok
    ...
    demo3() {
      this.$store.commit(ADD_COUNT, 15);
    },
    demo4() {
    ...
    //组合action测试:
    demo8() {
      const t = this.$store.dispatch("incrementAsync");
      console.log("dispatch over", t);
    },
    demo9() {
      this.$store.dispatch("incrementAsync").then((data: any) => {
        console.log("dipatch ,then over", data);
      });
    },
    async demo10() {
      const t = await this.$store.dispatch("incrementAsync");
    ...
    //类似地,映射Mutation到本地
    ...mapMutations([
      //this.increment与store.commit('increment')映射
      INCREMENT,
      //this.addTodo与store.commit('addTodo')映射
      ADD_TODO,
    ]),
    ...mapMutations({
      //this.add_count与store.commit('addCount')映射
      add_count: ADD_COUNT,
      //this.add_using_object_style与 store.commit('addUsingObjectStyle')
      add_using_object_style: ADD_USING_OBJECT_STYLE,
    }),

    //类似第,映射Action到本地,注意不要重名
    ...mapActions([
