...
<template>
  <accordion-hull  header="Class 与 Style 绑定">
    <accordion-item caption="class 的对象语法" show>
      <ul>
        <li>
          <span :class="{ active:judges.isActive }">传给:class一个对象{active:isActive}</span>
        </li>
        <li>
          <span :class="{active:!judges.isActive}">上述布尔字段取反得到</span>
        </li>
        <li>
          <span :class="{active:judges.isActive,'red-border':judges.isRedBorder}" class="my-italic">
            对象包含多个class属性绑定，并于普通class共存.渲染结果时class="active red-border my-italic"
          </span>
        </li>
        <li>
          <span :class="judges.obj_class">可以使用对象打包各个类名key和对应boolean值</span>
        </li>
        <li>
    ...
    <accordion-item caption="class 数组语法">
      <ul>
        <li>
          <span :class="[objCns.acc,objCns.rbc,objCns.itc]">
            传递多个元素组成的临时数组给:class,数组元素的值时具体的css类名称
          </span>
        </li>
        <li>
        ...
        <li>
          <span :class="['active','red-border','my-italic']">放屁脱裤子的数组传递方式</span>
        </li>
        <li>
          <span :class="[judges.isRedBorder ? objCns.rbc : arrCns[3], objCns.itc]">三元运算符结合属性组成的临时数组</span>
        </li>
        <li>
          <span :class="[{active:judges.isActive,'red-border':judges.isRedBorder},objCns.itc]">
    ...
    <accordion-item caption="Style的对象语法">
      <ul>
        <li>
          <span :style="{color:objStyle.clr,fontSize:objStyle.fs + 'px','background-color':objStyle.bgclr}">
...
<script lang="ts">
  ...
  //export default class ComOrViewModel extends Vue { }
