<template>
  <div ref="root">
    ...
    >
      ...
      <accordion-item caption="多个命名视图，包括默认视图">
        <router-link to="/exercise/route-params-to-props/two/hello-multiple-view" />
      ...
      <accordion-item caption="路由设置时，包含为指定的path内定好的props.同时演练过渡">
      ...
      <accordion-item caption="函数模式">
        <ul>
          <li>
            使用路径参数：
            <router-link to="/exercise/route-params-to-props/five/1919/Miniton" />
          </li>
          <li>
            使用查询参数：
            <router-link to="/exercise/route-params-to-props/six?x=7912&y=hey" />
          </li>
          <li>
            six页，仅查询参数变化
            <router-link to="/exercise/route-params-to-props/six?x=1010&y=world" />
          </li>
          <li>
...
export default defineComponent({
  ...
  setup() {
    ...
    router.beforeEach(async (to, from /*next*/) => {
      ...
      //meta测试：
      print(debug, "非递归合并所有 meta 字段的（从父字段到子字段）：", to.meta);

      //return action;
      return true; //演练与其他卫士钩子的时序。
    });
    //全局解析守卫, 最好在main.ts中以 use(...)注册插件的方式进行
    //确保在导航被确认之前，同时在所有组件内守卫和异步路由组件被解析之后，解析守卫就被正确调用
    router.beforeResolve(() => {
      print(debug, "05-params-to-props.vue router.beforeResolve");
    });
    //全局后置钩子,不会接受 next 函数也不会改变导航本身
    router.afterEach((to, from, failures) => {
      print(debug, "05-params-to-props.vue router.afterEach:" /* to, from, failures*/);
    });
    router.onError(() => {
