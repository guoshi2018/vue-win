<template>
  <div>
    ...
    <h4>cap1 is &lt;{{ cap1 }}&gt;</h4>
    <h5>cap2 is &lt;{{ cap2 }}&gt;</h5>
    <div>{{ readersNumber }},{{ book1 }},{{ book2 }}</div>
  </div>
</template>

<script lang="ts">
import { ref, toRef, toRefs, reactive, defineComponent } from "vue";
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    title.value += "....in setup"; //Set operation on key "title" failed: target is readonly
    print(debug, ";;;;;;;;;;;;;;;;title.value;;;;;;;;;;;;;;;;;;;", title.value);

    //cap1是可选的prop,则上述的props中可能没有title(实际是:当消费端未提供cap1,props.cap.value=undefined)
    //若果还是仿上结构,则得到undefined.
    const { cap1 } = toRefs(props);
    print(debug, cap1.value); //undefined

    //可以使用toRef,例如cap2.感觉其实与上面结果一样
    const cap2 = toRef(props, "cap2");
    print(debug, cap2, cap2.value);

    print(debug, "context参数,非响应式  -----------------------------");
    print(debug, context.attrs); // Attribute (非响应式对象，等同于 $attrs)
    print(debug, context.slots); // 插槽 (非响应式对象，等同于 $slots)
    print(debug, context.emit); // 触发事件 (方法，等同于 $emit)
    print(debug, context.expose); // 暴露公共 property (函数)
